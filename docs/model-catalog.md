...
组件渲染测试覆盖状态/草稿读取失败、启动与 repair 诊断、pending 恢复、健康但未应用指针、非法草稿修复、表单与 raw 往返、改名及条目删除冲突、跨浏览器保存与放弃失败、延迟保存响应、TOML 在 loading/error/success 下的原文保留。使用真实 hook、QueryClient 与生成 SDK，仅替换 HTTP 和 Monaco 边界；字段通过关联 label 定位，不依赖样式或 DOM 位置。
...
状态查询失败不表示没有用户级覆盖，草稿查询失败也不表示没有草稿；查询错误会显示，缺少当前证据的来源断言会撤下。新增条目的模板在选择时复制，字段集只从该副本推导；远端模板刷新不会给已打开的表单添加字段。
